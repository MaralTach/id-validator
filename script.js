...
form.addEventListener("submit", function (event) {
  ...
  if (
    idNumberInput.value.length === 11 &&
    isValidSSN(ssnInput.value) &&
    agreeCheck.checked
  ) {
    // All conditions are met, redirect to the new page
    window.location.href = "https://clarusway.com/";
  } else {
    // If any condition is not met, prevent form submission
    event.preventDefault();
  }
});

// Function to check validity of SSN
function isValidSSN(ssn) {
  const ssnPattern = /^(?!000|666|9\d\d)\d{3}-(?!00)\d{2}-(?!0000)\d{4}$/;
  return ssnPattern.test(ssn);
}
